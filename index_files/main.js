...
async function loadElectricityPrices() {
    ...
    const priceTimeZone = "Europe/Helsinki";
    ...
    const weatherKeyFormatter = new Intl.DateTimeFormat("sv-SE", {
        timeZone: priceTimeZone,
        year: "numeric",
        month: "2-digit",
        day: "2-digit",
    ...
    const weatherKey = date => weatherKeyFormatter.format(date).replace(" ", "T");

    if (!chart || !display) return;

    try {
        console.log("Fetching electricity prices...");
        // Fetch with cache buster to avoid GitHub Pages/browser stale JSON.
        ...
        const timeStr = `${timeFormatter.format(startT)}–${timeFormatter.format(endT)}`;
        display.textContent = `${currentP.price.toFixed(2)} snt · ${timeStr}`;

        // Fetch weather for Oitti, Hausjärvi (60.785, 24.832)
        let weatherData = null;
        try {
            const weatherRes = await fetch(`https://api.open-meteo.com/v1/forecast?latitude=60.785&longitude=24.832&hourly=temperature_2m,weathercode&timezone=${encodeURIComponent(priceTimeZone)}&forecast_days=2`);
            if (weatherRes.ok) {
        ...
        const barsHtml = upcomingPrices.map(p => {
            ...
            return `
                <div class="price-bar ${isCurrent ? 'is-current' : ''} ${isNegative ? 'is-negative' : ''}" 
                     ...
                     title="${timeFormatter.format(start)} - ${timeFormatter.format(end)}: ${p.price.toFixed(2)} snt">
                </div>
            `;
        }).join("");
        ...
        const markersHtml = upcomingPrices.map((p, index) => {
            ...
            if (start.getMinutes() === 0) {
                const leftPercent = (index / upcomingPrices.length) * 100;
                return `
                    <div class="price-hour-marker" style="left: ${leftPercent.toFixed(1)}%">
                        <span class="price-hour-label">${hourFormatter.format(start)}</span>
                    </div>
                `;
            }
            return "";
        ...
        if (weatherRow && weatherData) {
            weatherRow.innerHTML = upcomingPrices.map(p => {
                ...
                if (start.getMinutes() === 0) {
                    const wIndex = weatherData.hourly.time.indexOf(weatherKey(start));
